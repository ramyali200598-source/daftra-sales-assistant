...
const rates={annual:{user:315,branch:1860,warehouse:540,employee:99,offlineDevice:450,rentalUnit:90},monthly:{user:35,branch:195,warehouse:58.5,employee:11.25,offlineDevice:48.75,rentalUnit:9.38}};
...
function whatsAppText(){const {selected}=recommendation(),requirements=customerRequirements(),price=displayPrice(selected),extras=selected.extras,rate=rates[billingPeriod],plan=selected.plan,periodLabel=billingPeriod==='annual'?'سنوياً':'شهرياً';
...
const costLines=[
 `السعر الأساسي لباقة ${plan.name}: ${currency(selected.base)} ر.س`,
 extras.users>0?`تكلفة المستخدمين الإضافيين (${arDigits(extras.users)} × ${currency(rate.user)}): ${currency(extras.users*rate.user)} ر.س`:'',
 extras.branches>0?`تكلفة الفروع الإضافية (${arDigits(extras.branches)} × ${currency(rate.branch)}): ${currency(extras.branches*rate.branch)} ر.س`:'',
 extras.warehouses>0?`تكلفة المستودعات الإضافية (${arDigits(extras.warehouses)} × ${currency(rate.warehouse)}): ${currency(extras.warehouses*rate.warehouse)} ر.س`:'',
 extras.employees>0?`تكلفة الموظفين الإضافيين (${arDigits(extras.employees)} × ${currency(rate.employee)}): ${currency(extras.employees*rate.employee)} ر.س`:'',
 extras.offlineDevices>0?`تكلفة أجهزة نقاط البيع بدون إنترنت الإضافية (${arDigits(extras.offlineDevices)} × ${currency(rate.offlineDevice)}): ${currency(extras.offlineDevices*rate.offlineDevice)} ر.س`:'',
 extras.rentalUnits>0?`تكلفة وحدات الإيجار الإضافية (${arDigits(extras.rentalUnits)} × ${currency(rate.rentalUnit)}): ${currency(extras.rentalUnits*rate.rentalUnit)} ر.س`:'',
 price.planDiscount>0?`خصم الباقة (${arDigits(price.planDiscount)}%): -${currency(price.planDiscountAmount)} ر.س`:'',
 price.addonsDiscount>0?`خصم الإضافات (${arDigits(price.addonsDiscount)}%): -${currency(price.addonsDiscountAmount)} ر.س`:''
].filter(Boolean);
const totalLine=vatEnabled()?`ضريبة القيمة المضافة (VAT 15%): ${currency(price.vat)} ر.س\nالإجمالي العام: ${currency(price.total)} ر.س (شامل ضريبة القيمة المضافة VAT).`:`الإجمالي العام: ${currency(price.discounted)} ر.س (السعر غير شامل ضريبة القيمة المضافة VAT).`;
