...
const plansByCountry={
 'السعودية':[
  {name:'الأساسية',annual:1140,monthly:120,users:1,warehouses:1,invoices:100,employees:1,manufacturing:false,offlinePos:false,offlineDevicesIncluded:0,rentalUnitsIncluded:3,customers:100,storageGb:5,aiCoins:60,cashAccounts:2,einvoicePhase:'المرحلة الأولى'},
  {name:'المتقدمة',annual:1860,monthly:220,users:2,warehouses:3,invoices:500,employees:2,manufacturing:false,offlinePos:false,offlineDevicesIncluded:0,rentalUnitsIncluded:6,customers:300,storageGb:10,aiCoins:200,cashAccounts:5,einvoicePhase:'المرحلة الأولى والثانية'},
  {name:'الشاملة',annual:2700,monthly:320,users:2,warehouses:5,invoices:Infinity,employees:3,manufacturing:true,offlinePos:true,offlineDevicesIncluded:1,rentalUnitsIncluded:10,customers:Infinity,storageGb:20,aiCoins:500,cashAccounts:Infinity,einvoicePhase:'المرحلة الأولى والثانية'}
 ],
 'مصر':[
  {name:'الأساسية',annual:5874,monthly:733,users:1,warehouses:1,invoices:100,employees:0,manufacturing:false,offlinePos:false,offlineDevicesIncluded:0,rentalUnitsIncluded:3,customers:100,storageGb:5,aiCoins:60,cashAccounts:2,einvoicePhase:'متوافقة مع مصلحة الضرائب المصرية'},
  {name:'المتقدمة',annual:11731,monthly:1225,users:1,warehouses:3,invoices:500,employees:0,manufacturing:false,offlinePos:false,offlineDevicesIncluded:0,rentalUnitsIncluded:6,customers:300,storageGb:10,aiCoins:200,cashAccounts:5,einvoicePhase:'متوافقة مع مصلحة الضرائب المصرية'},
  {name:'الشاملة',annual:23520,monthly:2448,users:1,warehouses:5,invoices:Infinity,employees:0,manufacturing:true,offlinePos:true,offlineDevicesIncluded:1,rentalUnitsIncluded:10,customers:Infinity,storageGb:20,aiCoins:500,cashAccounts:Infinity,einvoicePhase:'متوافقة مع مصلحة الضرائب المصرية'}
 ]
};
const ratesByCountry={
 'السعودية':{annual:{user:315,branch:1860,warehouse:540,employee:99,offlineDevice:450,rentalUnit:90},monthly:{user:35,branch:195,warehouse:58.5,employee:11.25,offlineDevice:48.75,rentalUnit:9.38}},
 'مصر':{annual:{user:3528,branch:11760,warehouse:5880,employee:1293.6,offlineDevice:5880,rentalUnit:1176},monthly:{user:392,branch:1293.6,warehouse:637,employee:129.36,offlineDevice:637,rentalUnit:122.5}}
};
const currencyByCountry={'السعودية':'ر.س','مصر':'ج.م'};
const vatRateByCountry={'السعودية':0.15,'مصر':0.14};
const vatRate=()=>vatRateByCountry[activeCountry()]??0.15;
const activeCountry=()=>byId('country').value;
const activePlans=()=>plansByCountry[activeCountry()]||plansByCountry['السعودية'];
const activeRates=()=>ratesByCountry[activeCountry()]||ratesByCountry['السعودية'];
const currencyLabel=()=>currencyByCountry[activeCountry()]||'ر.س';
...
function planFeatures(plan){const f=(n)=>n===Infinity?'غير محدود':arDigits(n);return[
 ...
 `${arDigits(plan.rentalUnitsIncluded)} وحدة إيجار/تأجير متضمنة (شاليهات، سيارات، معدات...) — أي وحدة أكتر بمقابل`,
 plan.offlinePos?`نقاط بيع بدون إنترنت: ${arDigits(plan.offlineDevicesIncluded)} جهاز متضمن — أي جهاز إضافي بمقابل`:'بدون نقاط بيع بدون إنترنت (متاحة في الشاملة فقط)',
...
function customerRequirements(){const usersEntered=Math.max(0,number('users')),includesAdmin=byId('usersIncludeAdmin').value==='yes',totalUsers=includesAdmin?Math.max(1,usersEntered):usersEntered+1;return{users:usersEntered,totalUsers,branches:Math.max(1,number('branches')),warehouses:Math.max(1,number('warehouses')),invoices:number('invoices'),employees:Math.max(0,number('employees')),offlineDevices:Math.max(0,number('offlineDevices')),cashAccounts:Math.max(0,number('cashAccounts')),rentalUnits:Math.max(0,number('rentalUnits')),manufacturing:byId('manufacturing').checked};}
function limitations(plan,index,requirements){const reasons=[];if(requirements.manufacturing&&!plan.manufacturing)reasons.push('التصنيع / BOM مطلوب وهو غير متاح');if(requirements.offlineDevices>0&&!plan.offlinePos)reasons.push('نقاط البيع بدون إنترنت (Offline POS) غير متاحة في هذه الباقة — متاحة في الشاملة فقط');if(index===0&&requirements.branches>1)reasons.push(`عدد الفروع (${arDigits(requirements.branches)}) يتجاوز حد فرع واحد`);if(requirements.invoices>plan.invoices&&plan.invoices!==Infinity)reasons.push(`الفواتير (${arDigits(requirements.invoices)}) تتجاوز الحد (${arDigits(plan.invoices)})`);if(requirements.cashAccounts>plan.cashAccounts&&plan.cashAccounts!==Infinity)reasons.push(`حسابات الخزينة والبنوك (${arDigits(requirements.cashAccounts)}) تتجاوز الحد (${arDigits(plan.cashAccounts)})`);return reasons;}
function planQuote(plan,index,requirements){const rate=activeRates()[billingPeriod],base=plan[billingPeriod],totalUsers=requirements.totalUsers,extraUsers=Math.max(0,totalUsers-plan.users),extraBranches=Math.max(0,requirements.branches-1),extraWarehouses=Math.max(0,requirements.warehouses-plan.warehouses),extraEmployees=Math.max(0,requirements.employees-plan.employees),extraOfflineDevices=plan.offlinePos?Math.max(0,requirements.offlineDevices-plan.offlineDevicesIncluded):0,extraRentalUnits=Math.max(0,requirements.rentalUnits-plan.rentalUnitsIncluded);const cur=currencyLabel();const line=(qty,unit,label)=>`${arDigits(qty)} ${label} إضافي × ${currency(unit)} = ${currency(qty*unit)} ${cur}`;const addonItems=[extraUsers>0&&{label:line(extraUsers,rate.user,'مستخدم'),cost:extraUsers*rate.user},extraBranches>0&&{label:line(extraBranches,rate.branch,'فرع'),cost:extraBranches*rate.branch},extraWarehouses>0&&{label:line(extraWarehouses,rate.warehouse,'مستودع'),cost:extraWarehouses*rate.warehouse},extraEmployees>0&&{label:line(extraEmployees,rate.employee,'موظف'),cost:extraEmployees*rate.employee},extraOfflineDevices>0&&{label:line(extraOfflineDevices,rate.offlineDevice,'جهاز نقاط بيع بدون إنترنت'),cost:extraOfflineDevices*rate.offlineDevice},extraRentalUnits>0&&{label:line(extraRentalUnits,rate.rentalUnit,'وحدة إيجار'),cost:extraRentalUnits*rate.rentalUnit}].filter(Boolean);const addons=addonItems.reduce((s,i)=>s+i.cost,0);return{plan,index,base,addons,addonItems,extras:{users:extraUsers,branches:extraBranches,warehouses:extraWarehouses,employees:extraEmployees,offlineDevices:extraOfflineDevices,rentalUnits:extraRentalUnits},preTax:base+addons,limitations:limitations(plan,index,requirements)};}
function quotes(){const requirements=customerRequirements();return activePlans().map((plan,index)=>planQuote(plan,index,requirements));}
...
function displayPrice(quote){const planDiscount=discountPlanPercent(),addonsDiscount=discountAddonsPercent();const planDiscountAmount=quote.base*planDiscount/100,addonsDiscountAmount=quote.addons*addonsDiscount/100,discountAmount=planDiscountAmount+addonsDiscountAmount,discounted=quote.preTax-discountAmount,vat=discounted*vatRate();return{planDiscount,addonsDiscount,planDiscountAmount,addonsDiscountAmount,discountAmount,discounted,vat,total:discounted+vat,shown:vatEnabled()?discounted+vat:discounted};}
function priceDetails(quote){const price=displayPrice(quote),cur=currencyLabel();const addonsHtml=quote.addonItems.length?`<div class="addons">${quote.addonItems.map(i=>`+ ${i.label}`).join('<br>')}</div>`:'';const subtotalLine=quote.addonItems.length?`<br>الإجمالي قبل الضريبة: ${currency(quote.preTax)} ${cur}`:'';const discountLines=[price.planDiscount>0?`خصم الباقة ${arDigits(price.planDiscount)}%: -${currency(price.planDiscountAmount)} ${cur}`:'',price.addonsDiscount>0?`خصم الإضافات ${arDigits(price.addonsDiscount)}%: -${currency(price.addonsDiscountAmount)} ${cur}`:''].filter(Boolean);const discountHtml=discountLines.length?`<br><span class="addons">${discountLines.join('<br>')}</span><br>بعد الخصم: ${currency(price.discounted)} ${cur}`:'';return vatEnabled()?`<div class="price-details">السعر الأساسي: ${currency(quote.base)} ${cur}<br>${addonsHtml}${subtotalLine}${discountHtml}<br>VAT (${arDigits(Math.round(vatRate()*100))}%): ${currency(price.vat)} ${cur}<br><b>الإجمالي شامل الضريبة: ${currency(price.total)} ${cur}</b></div>`:`<div class="price-details">السعر الأساسي: ${currency(quote.base)} ${cur}<br>${addonsHtml}${discountHtml}<br><b>الإجمالي: ${currency(price.discounted)} ${cur}</b><br>الأسعار لا تشمل الضريبة</div>`;}
...
function whatsAppText(){const {selected}=recommendation(),requirements=customerRequirements(),price=displayPrice(selected),extras=selected.extras,rate=activeRates()[billingPeriod],plan=selected.plan,periodLabel=billingPeriod==='annual'?'سنوياً':'شهرياً',cur=currencyLabel();
const componentLines=[
 `المستخدمين (إجمالي ${arDigits(requirements.totalUsers)}): الباقة تتضمن ${arDigits(plan.users)} مستخدم${extras.users>0?` + ترقية لـ ${arDigits(extras.users)} مستخدم إضافي`:' فقط'}.`,
 `الفروع (إجمالي ${arDigits(requirements.branches)}): الباقة تتضمن فرع واحد${extras.branches>0?` + ترقية لـ ${arDigits(extras.branches)} فرع إضافي`:''}.`,
 `المستودعات (إجمالي ${arDigits(requirements.warehouses)}): الباقة تتضمن ${arDigits(plan.warehouses)} مستودع${extras.warehouses>0?` + ترقية لـ ${arDigits(extras.warehouses)} مستودع إضافي`:' فقط'}.`,
 `الموظفين (إجمالي ${arDigits(requirements.employees)}): الباقة تتضمن ${arDigits(plan.employees)} موظف${extras.employees>0?` + ترقية لـ ${arDigits(extras.employees)} موظف إضافي`:' فقط'}.`,
 requirements.offlineDevices>0?`أجهزة نقاط بيع بدون إنترنت (إجمالي ${arDigits(requirements.offlineDevices)}): الباقة تتضمن ${arDigits(plan.offlineDevicesIncluded)} جهاز${extras.offlineDevices>0?` + ترقية لـ ${arDigits(extras.offlineDevices)} جهاز إضافي`:' فقط'}.`:'',
 requirements.rentalUnits>0?`وحدات الإيجار/التأجير (إجمالي ${arDigits(requirements.rentalUnits)}): الباقة تتضمن ${arDigits(plan.rentalUnitsIncluded)} وحدة${extras.rentalUnits>0?` + ترقية لـ ${arDigits(extras.rentalUnits)} وحدة إضافية`:' فقط'}.`:'',
 requirements.manufacturing?'التصنيع / إدارة قوائم المواد (BOM): متاحة ضمن الباقة.':''
].filter(Boolean);
const costLines=[
 `السعر الأساسي لباقة ${plan.name}: ${currency(selected.base)} ${cur}`,
 extras.users>0?`تكلفة المستخدمين الإضافيين (${arDigits(extras.users)} × ${currency(rate.user)}): ${currency(extras.users*rate.user)} ${cur}`:'',
 extras.branches>0?`تكلفة الفروع الإضافية (${arDigits(extras.branches)} × ${currency(rate.branch)}): ${currency(extras.branches*rate.branch)} ${cur}`:'',
 extras.warehouses>0?`تكلفة المستودعات الإضافية (${arDigits(extras.warehouses)} × ${currency(rate.warehouse)}): ${currency(extras.warehouses*rate.warehouse)} ${cur}`:'',
 extras.employees>0?`تكلفة الموظفين الإضافيين (${arDigits(extras.employees)} × ${currency(rate.employee)}): ${currency(extras.employees*rate.employee)} ${cur}`:'',
 extras.offlineDevices>0?`تكلفة أجهزة نقاط البيع بدون إنترنت الإضافية (${arDigits(extras.offlineDevices)} × ${currency(rate.offlineDevice)}): ${currency(extras.offlineDevices*rate.offlineDevice)} ${cur}`:'',
 extras.rentalUnits>0?`تكلفة وحدات الإيجار الإضافية (${arDigits(extras.rentalUnits)} × ${currency(rate.rentalUnit)}): ${currency(extras.rentalUnits*rate.rentalUnit)} ${cur}`:'',
 price.planDiscount>0?`خصم الباقة (${arDigits(price.planDiscount)}%): -${currency(price.planDiscountAmount)} ${cur}`:'',
 price.addonsDiscount>0?`خصم الإضافات (${arDigits(price.addonsDiscount)}%): -${currency(price.addonsDiscountAmount)} ${cur}`:''
].filter(Boolean);
const totalLine=vatEnabled()?`ضريبة القيمة المضافة (VAT ${arDigits(Math.round(vatRate()*100))}%): ${currency(price.vat)} ${cur}\nالإجمالي العام: ${currency(price.total)} ${cur} (شامل ضريبة القيمة المضافة VAT).`:`الإجمالي العام: ${currency(price.discounted)} ${cur} (السعر غير شامل ضريبة القيمة المضافة VAT).`;
return `مرحباً ${byId('customerName').value||'أستاذ/ة'}،\n\nبناءً على دراسة متطلباتكم، نوصي بالاعتماد على "باقة ${plan.name}"، وتجدون أدناه تفصيل المكونات الأساسية والإضافات المخصصة لتلبية احتياجات حجم أعمالكم:\n\nتفاصيل الصلاحيات والمكونات:\n${componentLines.join('\n')}\n\nتفاصيل الاستثمار المالي (${periodLabel}):\n${costLines.join('\n')}\n${totalLine}\n\nيسعدنا استكمال الخطوات القادمة وتفعيل الحساب، ونحن جاهزون لأي استفسار.\n\nتحياتي،`;}
function buildProposal(){const {selected}=recommendation(),req=customerRequirements(),price=displayPrice(selected),cur=currencyLabel();const discountRows=[price.planDiscount>0?`<tr><td>خصم الباقة (${arDigits(price.planDiscount)}%)</td><td>-${currency(price.planDiscountAmount)} ${cur}</td></tr>`:'',price.addonsDiscount>0?`<tr><td>خصم الإضافات (${arDigits(price.addonsDiscount)}%)</td><td>-${currency(price.addonsDiscountAmount)} ${cur}</td></tr>`:''].join('');const offlinePosRow=req.offlineDevices>0?`<tr><td>نقاط بيع بدون إنترنت</td><td>${arDigits(req.offlineDevices)} جهاز</td></tr>`:'';const rentalUnitsRow=req.rentalUnits>0?`<tr><td>وحدات الإيجار/التأجير</td><td>${arDigits(req.rentalUnits)} وحدة</td></tr>`:'';return `<header><h1 style="color:#0e3d36">دفتـرة ERP</h1><p>عرض تجاري · ${new Date().toLocaleDateString('ar-SA')}</p></header><h2>عرض إلى: ${byId('customerName').value}</h2><p>نوصي بباقة <b>${selected.plan.name}</b> لأنها أقل باقة مؤهلة تحقق المتطلبات.</p><table><tr><th>البند</th><th>التفاصيل</th></tr><tr><td>المستخدمون والفروع</td><td>${arDigits(req.totalUsers)} مستخدم (شامل الأدمن) · ${arDigits(req.branches)} فرع · ${arDigits(req.warehouses)} مستودع</td></tr><tr><td>الموظفون (الموارد البشرية)</td><td>${arDigits(req.employees)} موظف</td></tr>${offlinePosRow}${rentalUnitsRow}<tr><td>الوحدات</td><td>المبيعات، المحاسبة، المخزون، CRM ${req.manufacturing?'، التصنيع':''}</td></tr><tr><td>السعر الأساسي</td><td>${currency(selected.preTax)} ${cur}</td></tr>${discountRows}${vatEnabled()?`<tr><td>ضريبة القيمة المضافة (${arDigits(Math.round(vatRate()*100))}%)</td><td>${currency(price.vat)} ${cur}</td></tr><tr><th>الإجمالي شامل الضريبة</th><th>${currency(price.total)} ${cur}</th></tr>`:`<tr><th>الإجمالي قبل الضريبة</th><th>${currency(price.discounted)} ${cur}</th></tr>`}</table><p>الشروط: الأسعار خاضعة للمراجعة والاعتماد النهائي من دفتـرة.</p><button class="btn primary" onclick="print()">تصدير PDF / طباعة</button> <button class="btn" onclick="proposalModal.classList.remove('show')">إغلاق</button>`;}

/* ROI Calculator */
function renderRoi(monthlyCost){const hours=number('wastedHours')*number('roiEmployees')*4,saved=hours*(number('averageSalary')/160);byId('timeSaved').textContent=currency(hours)+' ساعة';byId('moneySaved').textContent=currency(saved)+' '+currencyLabel();byId('roiValue').textContent=monthlyCost?arDigits(Math.round(saved/monthlyCost*100))+'%':'—';}

function renderActivityInsight(){const key=byId('activity').value,data=activityInsights[key],card=byId('activityInsightCard');if(!data){card.hidden=true;return;}const block=(title,icon,list)=>`<div class="ai-block"><h4>${icon} ${title}</h4><ul>${list.map(item=>`<li>${item}</li>`).join('')}</ul></div>`;byId('activityInsightBody').innerHTML=block('نقاط الألم','🩹',data.pain)+block('الحل التقني في دفترة','🛠️',data.solution)+block('ثغرة المنافس','🎯',data.competitorGap);card.hidden=false;}
...
function render(){renderPlans();renderDashboard();renderAssistant();renderActivityInsight();const message=whatsAppText();byId('whatsAppMessage').value=message;byId('whatsAppLink').href='https://wa.me/?text='+encodeURIComponent(message);byId('vatDescription').textContent=vatEnabled()?'يعرض الأساسي والضريبة والإجمالي':'يعرض السعر الأساسي فقط';saveDraft();}
