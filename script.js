...
function priceDetails(quote){const price=displayPrice(quote.preTax);const addonsHtml=quote.addonItems.length?`<div class="addons">${quote.addonItems.map(i=>`+ ${i.label}`).join('<br>')}</div>`:'';const subtotalLine=quote.addonItems.length?`<br>الإجمالي قبل الضريبة: ${currency(price.base)} ر.س`:'';return vatEnabled()?`<div class="price-details">السعر الأساسي: ${currency(quote.base)} ر.س<br>${addonsHtml}${subtotalLine}<br>VAT (15%): ${currency(price.vat)} ر.س<br><b>الإجمالي شامل الضريبة: ${currency(price.total)} ر.س</b></div>`:`<div class="price-details">السعر الأساسي: ${currency(quote.base)} ر.س<br>${addonsHtml}<br><b>الإجمالي: ${currency(price.base)} ر.س</b><br>الأسعار لا تشمل الضريبة</div>`;}
