...
function customerRequirements(){return{users:Math.max(0,number('users')),branches:Math.max(1,number('branches')),warehouses:Math.max(1,number('warehouses')),invoices:number('invoices'),employees:Math.max(0,number('employees')),offlineDevices:Math.max(0,number('offlineDevices')),cashAccounts:Math.max(0,number('cashAccounts')),rentalUnits:Math.max(0,number('rentalUnits')),manufacturing:byId('manufacturing').checked};}
...
const componentLines=[
 `المستخدمين (إجمالي ${arDigits(requirements.users+1)}): الباقة تتضمن ${arDigits(plan.users)} مستخدم${extras.users>0?` + ترقية لـ ${arDigits(extras.users)} مستخدم إضافي`:' فقط'}.`,
